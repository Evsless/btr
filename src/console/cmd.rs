...
pub struct CommandNode {
    ...
    pub handler: Option<fn(&mut TrackerManager, &[&str]) -> Result<(), BtrError>>,
    extra_args: bool
}

impl CommandNode {
    pub fn new(cmd: &str, description: &str, handler: Option<fn(&mut TrackerManager, &[&str]) -> Result<(), BtrError>>) -> Self {
        Self {
            cmd: cmd.to_string(),
